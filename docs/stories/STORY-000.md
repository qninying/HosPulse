...
**HosPulse** — Early-warning intelligence system for rural and Critical Access Hospital management companies.
...
Who this is for and what they are trying to get done. Take the roles from your own stories — they are written "As a <role>, I want …". Roles in your plan: system admin, system operator, public user, operator, data operator.
...
- **REQ-006** — The system must reject any AI-generated briefing containing a number not in the input data.
- **REQ-008** — The system must enforce database row-level security so each management company only sees its own hospitals.
- **REQ-009** — The system must reject any file containing patient names, birth dates, or record numbers before it is stored or logged.
- **REQ-012** — The system must mark unknown formats as 'needs mapping' rather than guessing.
- **REQ-013** — The system must link every computed metric back to the file and row it came from.
- **REQ-015** — The system must ensure every import, upload, normalization, and email is idempotent.
...
- **System Admin** — owns STORY-001, STORY-004, STORY-006, STORY-007, STORY-009
- **system** — owns STORY-002, STORY-010, STORY-011, STORY-012
- **Public User** — owns STORY-003
- **Operator** — owns STORY-005, STORY-008
- **Data Operator** — owns STORY-013
...
- **REQ-001** (FUNC, must) — The system must import public CMS HCRIS Medicare cost report files for rural and Critical Access Hospitals in Oklahoma and Texas.
- **REQ-002** (FUNC, must) — The system must compute operating margin, days cash on hand, and days in A/R per hospital per year from the imported data.
- **REQ-003** (FUNC, must) — The system must provide a free public Health Snapshot showing three years of trends for each hospital.
- **REQ-004** (FUNC, must) — The system must flag hospitals with operating margin below 0%, days cash on hand below 30, or days in A/R rising two years in a row.
- **REQ-005** (FUNC, must) — The system must store the exact values that triggered each flag.
- **REQ-006** (SAFE, must) — The system must reject any AI-generated briefing containing a number not in the input data.
- **REQ-007** (FUNC, must) — The system must allow operators to upload monthly exports per hospital, including general ledger, A/R aging, denials summary, and staffing.
- **REQ-008** (SAFE, must) — The system must enforce database row-level security so each management company only sees its own hospitals.
- **REQ-009** (SAFE, must) — The system must reject any file containing patient names, birth dates, or record numbers before it is stored or logged.
- **REQ-010** (FUNC, must) — The system must detect duplicate uploads by file hash.
- **REQ-011** (FUNC, must) — The system must convert exports from different hospital systems into a standard set of monthly metrics.
- **REQ-012** (SAFE, must) — The system must mark unknown formats as 'needs mapping' rather than guessing.
- **REQ-013** (SAFE, must) — The system must link every computed metric back to the file and row it came from.
- **REQ-014** (FUNC, must) — The system must send a weekly email briefing every Monday at 6:00 AM Central.
- **REQ-015** (SAFE, must) — The system must ensure every import, upload, normalization, and email is idempotent.
- **REQ-016** (FUNC, must) — The system must surface a slipping hospital at least 4 weeks earlier than the operator's current reporting cycle.
- **REQ-017** (CONSTRAINT, must) — The system must use Supabase for auth, Postgres, and storage.
- **REQ-018** (CONSTRAINT, must) — The system must use Vercel for hosting and the Anthropic Claude API for AI.
- **REQ-019** (CONSTRAINT, must) — The system must use Resend for email delivery.
- **REQ-020** (CONSTRAINT, must) — The system must use Python for data work and Next.js for the web app.
